...
export class StoreEntity {
  static async list(): Promise<Store[]> {
    const res = await apiFetch(`${API_BASE()}/lojas`, { cache: "no-store" });
  ...
  static async create(data: {
    ...
    const res = await apiFetch(`${API_BASE()}/lojas`, {
      method: "POST",
  ...
  static async delete(idLoja: number): Promise<void> {
    const res = await apiFetch(`${API_BASE()}/lojas/${idLoja}`, { method: "DELETE" });
    if (!res.ok) {
      const text = await res.text();
